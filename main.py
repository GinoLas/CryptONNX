...
hmac_key = bytes.fromhex("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") 
            ...
            for input in node.inputs :
                if isinstance(input, gs.Constant):
                    ...
                    if(".weight" in input.name):
                        ...
                        digest = hmac.new(hmac_key, np.ascontiguousarray(encrypted_bytes), hashlib.sha256).digest()
